...
Namespace Microsoft.VisualBasic.CompilerServices
    ...
    Friend NotInheritable Class VBBinder
        ...
        Private Sub ThrowInvalidCast(ByVal ArgType As System.Type, ByVal ParmType As System.Type, ByVal ParmIndex As Integer)
            Throw New InvalidCastException(SRExt.Format(SR.InvalidCast_FromToArg4, CalledMethodName(), CStr(ParmIndex + 1), VBFriendlyName(ArgType), VBFriendlyName(ParmType)))
        End Sub
                        ...
                        For NameIndex = 0 To names.GetUpperBound(0)
                            ...
                            If (ParmIndex > LastParmIndexToCheck) Then
                                ...
                                If SelectedCount = 1 Then
                                    ...
                                    Throw New MissingMemberException(SRExt.Format(SR.Argument_InvalidNamedArg2, names(NameIndex), CalledMethodName()))
                                End If
            ...
            For MethodIndex = 0 To match.GetUpperBound(0)
                ...
                If Not ThisMethod Is Nothing Then
                    ...
                    If (ParamArrayIndex = PARAMARRAY_NONE) AndAlso (args.Length > Parameters.Length) Then
                        ...
                        If SelectedCount = 1 Then
                            Throw New MissingMemberException(SRExt.Format(SR.NoMethodTakingXArguments2, CalledMethodName(), CStr(GetPropArgCount(args, IsPropertySet))))
                        End If
...
ClearMethod7:
                If SelectedCount = 1 Then
                    ...
                    Else
                        Throw New AmbiguousMatchException(SRExt.Format(SR.AmbiguousMatch_NarrowingConversion1, CalledMethodName()))
                    End If
                End If
                match(MethodIndex) = Nothing
                SelectedCount -= 1
...
NextMethod8:
                ...
                If (AmbiguousCount > 1) Then
                    ...
                    Select Case LowestScore
                        Case BindScore.Exact
                            Throw New AmbiguousMatchException(SRExt.Format(SR.AmbiguousCall_ExactMatch2, CalledMethodName(), Msg))

                        Case BindScore.Widening0, BindScore.Widening1
                            Throw New AmbiguousMatchException(SRExt.Format(SR.AmbiguousCall_WideningConversion2, CalledMethodName(), Msg))

                        Case Else
                            'BindScore.Narrowing
                            'BindScore.Unknown
                            Throw New AmbiguousMatchException(SRExt.Format(SR.AmbiguousCall2, CalledMethodName(), Msg))
                    End Select
                End If
        ...
        Public Overrides Function ChangeType(ByVal value As Object, ByVal typ As Type, ByVal culture As CultureInfo) As Object
            ...
            Catch ex As Exception
                Throw New InvalidCastException(SRExt.Format(SR.InvalidCast_FromTo, VBFriendlyName(value), VBFriendlyName(typ)))
            End Try
        End Function
        ...
        Private Function CreateParamOrder(ByVal SetProp As Boolean, ByVal paramOrder() As Integer, ByVal pars() As ParameterInfo, ByVal args() As Object, ByVal names() As String) As Exception
            ...
            For i = 0 To names.GetUpperBound(0)
                ...
                For j = 0 To LastNonSetIndex
                    ...
                    If StrComp(names(i), pars(j).Name, CompareMethod.Text) = 0 Then
                        ...
                        If paramOrder(j) <> -1 Then
                            Return New ArgumentException(SRExt.Format(SR.NamedArgumentAlreadyUsed1, pars(j).Name))
                        End If
                ...
                If (j > LastNonSetIndex) Then
                    ...
                    Return New MissingMemberException(SRExt.Format(SR.Argument_InvalidNamedArg2, names(i), CalledMethodName()))
                End If
        ...
        Friend Function InvokeMember(ByVal name As String,
            ...
            If objType.IsCOMObject() Then
                ...
                Catch ex As MissingMemberException
                    Throw New MissingMemberException(SRExt.Format(SR.MissingMember_MemberNotFoundOnType2, name, VBFriendlyName(objType)))
                End Try
            End If
...
#End If
            ...
            If (name.Length = 0) Then
                If (objType Is objIReflect) Then
                    ...
                    If (name Is Nothing) Then
                        Throw New MissingMemberException(SRExt.Format(SR.MissingMember_NoDefaultMemberFound1, VBFriendlyName(objType)))
                    End If
                Else
            ...
            If (invokeMethod Is Nothing) Then
                Throw New MissingMemberException(SRExt.Format(SR.NoMethodTakingXArguments2, CalledMethodName(), CStr(GetPropArgCount(args, (invokeAttr And BindingFlags.SetProperty) <> 0))))
            End If
            ...
            For MemberIndex = 0 To mi.GetUpperBound(0)
                ...
                ElseIf ThisMember.MemberType = MemberTypes.NestedType Then
                    ...
                    If RemovedCount = mi.Length - 1 Then
                        Throw New ArgumentException(SRExt.Format(SR.Argument_IllegalNestedType2, name, VBFriendlyName(objType)))
                    End If
